...
Contains 
 ...
 Subroutine CalculateCouplingCT(Lam,v,LSH,g1,g2,TW,g3,Yd,ZDL,ZDR,Ye,ZEL,               & 
& ZER,Yu,ZUL,ZUR,dLam,dv,dLSH,dg1,dg2,dSinTW,dCosTW,dTanTW,dg3,dYd,dZDL,dZDR,            & 
& dYe,dZEL,dZER,dYu,dZUL,dZUR,ctcplAhAhhh,ctcplhhhhhh,ctcplhhHpcHp,ctcplhhssss,          & 
...
Complex(dp), Intent(in) :: Lam,LSH,Yd(3,3),ZDL(3,3),ZDR(3,3),Ye(3,3),ZEL(3,3),ZER(3,3),Yu(3,3),ZUL(3,3),         & 
& ZUR(3,3),dLam,dLSH,dYd(3,3),dZDL(3,3),dZDR(3,3),dYe(3,3),dZEL(3,3),dZER(3,3),          & 
...
ctcplhhssss = 0._dp 
Call CT_Couplinghhssss(LSH,v,dLSH,dv,ctcplhhssss)
...
Subroutine CT_Couplinghhssss(LSH,v,dLSH,dv,res)

Implicit None 

Real(dp), Intent(in) :: v,dv

Complex(dp), Intent(in) :: LSH,dLSH

Complex(dp), Intent(out) :: res 
 
Integer :: j1,j2,j3,j4,j5,j6, j7, j8, j9, j10, j11, j12 
Iname = Iname +1 
NameOfUnit(Iname) = 'CT_Couplinghhssss' 
...
End Subroutine CT_Couplinghhssss  
...
res = 0._dp 
res = res-1._dp/2._dp*(dSinTW*g1*g2*v)
res = res-1._dp/2._dp*(dv*g1*g2*Sin(TW))
res = res-1._dp/2._dp*(dg2*g1*v*Sin(TW))
res = res-1._dp/2._dp*(dg1*g2*v*Sin(TW))
If (Real(res,dp).ne.Real(res,dp)) Then 
 Write(*,*) "NaN appearing in ",NameOfUnit(Iname) 
 Call TerminateProgram 
End If 
