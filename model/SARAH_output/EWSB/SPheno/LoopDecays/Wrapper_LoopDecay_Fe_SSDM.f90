...
Module Wrapper_OneLoopDecay_Fe_SSDM
Use Model_Data_SSDM 
Use Kinematics 
Use OneLoopDecay_Fe_SSDM 
Use Control 
Use Settings 
...
& ZfVPVZ,ZfVZVP,cplAhAhhh,cplAhcHpVWp,cplAhhhVZ,cplAhHpcVWp,cplcFeFeAhL,cplcFeFeAhR,     & 
...
Call Amplitude_Tree_SSDM_FeToFehh(cplcFeFehhL,cplcFeFehhR,MFe,Mhh,MFe2,               & 
& Mhh2,AmpTreeFeToFehh)

  Else 
Call Amplitude_Tree_SSDM_FeToFehh(ZcplcFeFehhL,ZcplcFeFehhR,MFe,Mhh,MFe2,             & 
& Mhh2,AmpTreeFeToFehh)
...
If (OSkinematics) Then 
  If (.not.ExternalZfactors) Then 
 ! OS and no Z-factors 
Call Gamma_Real_SSDM_FeToFehh(MLambda,em,gs,cplcFeFehhL,cplcFeFehhR,MFeOS,            & 
& MhhOS,MRPFeToFehh,MRGFeToFehh)

  Else 
 ! OS and Z-factors 
Call Gamma_Real_SSDM_FeToFehh(MLambda,em,gs,ZcplcFeFehhL,ZcplcFeFehhR,MFeOS,          & 
& MhhOS,MRPFeToFehh,MRGFeToFehh)
...
Else 
 ! DR and no Z-factors 
  If (.not.ExternalZfactors) Then 
Call Gamma_Real_SSDM_FeToFehh(MLambda,em,gs,cplcFeFehhL,cplcFeFehhR,MFe,              & 
& Mhh,MRPFeToFehh,MRGFeToFehh)

  Else 
 ! DR and Z-factors 
Call Gamma_Real_SSDM_FeToFehh(MLambda,em,gs,ZcplcFeFehhL,ZcplcFeFehhR,MFe,            & 
& Mhh,MRPFeToFehh,MRGFeToFehh)
...
Call Amplitude_WAVE_SSDM_FeToFehh(cplcFeFehhL,cplcFeFehhR,ctcplcFeFehhL,              & 
& ctcplcFeFehhR,MFe,MFe2,Mhh,Mhh2,ZfEL,ZfER,Zfhh,AmpWaveFeToFehh)



!Vertex Corrections 
Call Amplitude_VERTEX_SSDM_FeToFehh(MAh,MFe,Mhh,MHp,MVP,MVWp,MVZ,MAh2,MFe2,           & 
...
If (ShiftIRdiv) Then 
Call Amplitude_IR_VERTEX_SSDM_FeToFehh(MAh,MFe,Mhh,MHp,MVP,MVWp,MVZ,MAh2,             & 
...
& cplhhcVWpVWp,cplhhVZVZ,AmpVertexIRdrFeToFehh)
 ...
 If (ExternalZfactors) Then 
  If (OSkinematics) Then 
 ! OS and Z-factors 
Call Amplitude_IR_VERTEX_SSDM_FeToFehh(MAhOS,MFeOS,MhhOS,MHpOS,MVP,MVWpOS,            & 
...
!Tree Level 
  If (.not.ExternalZfactors) Then 
Call Amplitude_Tree_SSDM_FeToFeVZ(cplcFeFeVZL,cplcFeFeVZR,MFe,MVZ,MFe2,               & 
& MVZ2,AmpTreeFeToFeVZ)

  Else 
Call Amplitude_Tree_SSDM_FeToFeVZ(ZcplcFeFeVZL,ZcplcFeFeVZR,MFe,MVZ,MFe2,             & 
& MVZ2,AmpTreeFeToFeVZ)
...
If (OSkinematics) Then 
  If (.not.ExternalZfactors) Then 
 ! OS and no Z-factors 
Call Gamma_Real_SSDM_FeToFeVZ(MLambda,em,gs,cplcFeFeVZL,cplcFeFeVZR,MFeOS,            & 
& MVZOS,MRPFeToFeVZ,MRGFeToFeVZ)

  Else 
 ! OS and Z-factors 
Call Gamma_Real_SSDM_FeToFeVZ(MLambda,em,gs,ZcplcFeFeVZL,ZcplcFeFeVZR,MFeOS,          & 
& MVZOS,MRPFeToFeVZ,MRGFeToFeVZ)
...
Else 
 ! DR and no Z-factors 
  If (.not.ExternalZfactors) Then 
Call Gamma_Real_SSDM_FeToFeVZ(MLambda,em,gs,cplcFeFeVZL,cplcFeFeVZR,MFe,              & 
& MVZ,MRPFeToFeVZ,MRGFeToFeVZ)

  Else 
 ! DR and Z-factors 
Call Gamma_Real_SSDM_FeToFeVZ(MLambda,em,gs,ZcplcFeFeVZL,ZcplcFeFeVZR,MFe,            & 
& MVZ,MRPFeToFeVZ,MRGFeToFeVZ)
...
!Self-energy Corrections 
Call Amplitude_WAVE_SSDM_FeToFeVZ(cplcFeFeVPL,cplcFeFeVPR,cplcFeFeVZL,cplcFeFeVZR,    & 
...
!Vertex Corrections 
Call Amplitude_VERTEX_SSDM_FeToFeVZ(MAh,MFe,Mhh,MHp,MVP,MVWp,MVZ,MAh2,MFe2,           & 
...
If (ShiftIRdiv) Then 
Call Amplitude_IR_VERTEX_SSDM_FeToFeVZ(MAh,MFe,Mhh,MHp,MVP,MVWp,MVZ,MAh2,             & 
...
& cplcHpVWpVZ,cplcVWpVWpVZ,AmpVertexIRdrFeToFeVZ)
 ...
 If (ExternalZfactors) Then 
  If (OSkinematics) Then 
 ! OS and Z-factors 
Call Amplitude_IR_VERTEX_SSDM_FeToFeVZ(MAhOS,MFeOS,MhhOS,MHpOS,MVP,MVWpOS,            & 
...
!Tree Level 
  If (.not.ExternalZfactors) Then 
Call Amplitude_Tree_SSDM_FeToFvcVWp(cplcFeFvcVWpL,cplcFeFvcVWpR,MFe,MVWp,             & 
& MFe2,MVWp2,AmpTreeFeToFvcVWp)

  Else 
Call Amplitude_Tree_SSDM_FeToFvcVWp(ZcplcFeFvcVWpL,ZcplcFeFvcVWpR,MFe,MVWp,           & 
& MFe2,MVWp2,AmpTreeFeToFvcVWp)
...
If (OSkinematics) Then 
  If (.not.ExternalZfactors) Then 
 ! OS and no Z-factors 
Call Gamma_Real_SSDM_FeToFvcVWp(MLambda,em,gs,cplcFeFvcVWpL,cplcFeFvcVWpR,            & 
& MFeOS,MVWpOS,MRPFeToFvcVWp,MRGFeToFvcVWp)

  Else 
 ! OS and Z-factors 
Call Gamma_Real_SSDM_FeToFvcVWp(MLambda,em,gs,ZcplcFeFvcVWpL,ZcplcFeFvcVWpR,          & 
& MFeOS,MVWpOS,MRPFeToFvcVWp,MRGFeToFvcVWp)
...
!Self-energy Corrections 
Call Amplitude_WAVE_SSDM_FeToFvcVWp(cplcFeFvcVWpL,cplcFeFvcVWpR,ctcplcFeFvcVWpL,      & 
& ctcplcFeFvcVWpR,MFe,MFe2,MVWp,MVWp2,ZfEL,ZfER,ZfvL,ZfVWp,AmpWaveFeToFvcVWp)



!Vertex Corrections 
Call Amplitude_VERTEX_SSDM_FeToFvcVWp(MAh,MFe,Mhh,MHp,MVP,MVWp,MVZ,MAh2,              & 
...
If (ShiftIRdiv) Then 
Call Amplitude_IR_VERTEX_SSDM_FeToFvcVWp(MAh,MFe,Mhh,MHp,MVP,MVWp,MVZ,MAh2,           & 
& MFe2,Mhh2,MHp2,MVP2,MVWp2,MVZ2,cplcFeFeAhL,cplcFeFeAhR,cplAhHpcVWp,cplcFeFehhL,        & 
& cplcFeFehhR,cplcFeFeVPL,cplcFeFeVPR,cplcFeFeVZL,cplcFeFeVZR,cplcFvFvVZL,               & 
& cplcFvFvVZR,cplcFeFvcHpL,cplcFeFvcHpR,cplcFeFvcVWpL,cplcFeFvcVWpR,cplhhHpcVWp,         & 
& cplhhcVWpVWp,cplHpcVWpVP,cplHpcVWpVZ,cplcVWpVPVWp,cplcVWpVWpVZ,AmpVertexIRdrFeToFvcVWp)
 ...
 If (ExternalZfactors) Then 
  If (OSkinematics) Then 
 ! OS and Z-factors 
Call Amplitude_IR_VERTEX_SSDM_FeToFvcVWp(MAhOS,MFeOS,MhhOS,MHpOS,MVP,MVWpOS,          & 
& MVZOS,MAh2OS,MFe2OS,Mhh2OS,MHp2OS,MVP2,MVWp2OS,MVZ2OS,cplcFeFeAhL,cplcFeFeAhR,         & 
...
!Self-energy Corrections 
 If (ExternalZfactors) Then 
Call Amplitude_WAVE_SSDM_FeToFeVP(ZcplcFeFeVPL,ZcplcFeFeVPR,ZcplcFeFeVZL,             & 
...
& MFe2OS,MVP,MVP2,ZfEL,ZfER,ZfVP,ZfVZVP,AmpWaveFeToFeVP)
 ...
 Else 
Call Amplitude_WAVE_SSDM_FeToFeVP(cplcFeFeVPL,cplcFeFeVPR,cplcFeFeVZL,cplcFeFeVZR,    & 
...
!Vertex Corrections 
 If (ExternalZfactors) Then 
Call Amplitude_VERTEX_SSDM_FeToFeVP(MAhOS,MFeOS,MhhOS,MHpOS,MVP,MVWpOS,               & 
& MVZOS,MAh2OS,MFe2OS,Mhh2OS,MHp2OS,MVP2,MVWp2OS,MVZ2OS,ZcplcFeFeAhL,ZcplcFeFeAhR,       & 
...
!Vertex Corrections 
Call Amplitude_VERTEX_SSDM_FeToFeVP(MAh,MFe,Mhh,MHp,MVP,MVWp,MVZ,MAh2,MFe2,           & 
...
End Module Wrapper_OneLoopDecay_Fe_SSDM
